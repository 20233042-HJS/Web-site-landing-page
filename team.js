...
async function loadInitialData() {
    try {
        const taskRes = await fetch(`http://localhost:3000/api/tasks?team=${currentTeam}`);
        if (taskRes.ok) {
        ...
        // 진행도 로드
        const progRes = await fetch(`http://localhost:3000/api/progress?team=${currentTeam}`);
        if (progRes.ok) {
...
async function loadSavedLinks() {
    try {
        const res = await fetch(`http://localhost:3000/api/links?team=${currentTeam}`);
        if (!res.ok) return;
