...
document.getElementById('questionForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) { loadQuestions(); toggleWriteForm(false); }
    } catch (error) { console.error('질문 등록 에러:', error); }
});
...
async function loadQuestions() {
    ...
    } catch (error) { console.error('목록 불러오기 에러:', error); }
}

async function likeQuestion(questionId) {
    try {
        const response = await fetch(`${API_BASE}/api/questions/${questionId}/like`, { method: 'PUT' });
        if (response.ok) {
...
async function deleteQuestion(questionId, btn) {
    ...
    try {
        const response = await fetch(`${API_BASE}/api/questions/${questionId}`, { method: 'DELETE' });
        if (response.ok) { btn.closest('.q-item').remove(); }
